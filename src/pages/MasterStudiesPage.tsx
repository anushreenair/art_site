...
export function MasterStudiesPage() {
  ...
  const [selectedId, setSelectedId] = useState(params.get('study') ?? 'vermeer-pearl');
  ...
  return <AppShell>
    ...
    <section id="selected-master" className="master-study" aria-label={`${selected.title} study`}><div className="master-original"><img src={selected.image} alt={selected.alt} /><span>{selected.movement}</span></div><div className="master-meta"><p className="eyebrow">The original</p><h2>{selected.title}</h2><p className="master-artist">{selected.artist} · {selected.year}</p><p className="master-medium">{selected.medium}</p><p>Use this original as a field of decisions. The goal is not a faithful copy—it is noticing what makes the image work.</p></div><div className="master-analysis"><p className="eyebrow">What to look for</p>{selected.analysis.map((item) => <article key={item.label}><h3>{item.label}</h3><p>{item.note}</p></article>)}</div><div className="master-activities"><header><div><p className="eyebrow">Practice activities</p><h2>Take one decision to the page.</h2></div><p>Short studies make room for looking again.</p></header>{selected.activities.map((activity, index) => <article key={activity.title}><span>{String(index + 1).padStart(2, '0')}</span><div><h3>{activity.title}</h3><p>{activity.note}</p></div><small>{activity.duration}</small><Link to={`/practice/${activity.studyId}`}>{activity.title === '15-Minute Value Study' ? '15-Minute Value Study' : 'Start practice'} <b>↗</b></Link></article>)}</div></section>
  </AppShell>;
}
