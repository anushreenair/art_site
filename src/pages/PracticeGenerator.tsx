...
export function PracticeGenerator() {
  ...
  const generate = () => { const next = buildPracticeSession(choices); setSession(next); setRemaining(next.seconds); setStatus('idle'); setGuidanceOpen(false); setSaved(false); };
  const restart = () => { if (!session) return; setRemaining(session.seconds); setStatus('idle'); };

  return <AppShell><section className="generator-intro"><p className="eyebrow">Personal practice generator</p><h1 aria-label="What do you want to practise?">What do you want<br />to <em>practise?</em></h1><p>Choose the work you have time for. We’ll shape it into one focused studio session.</p></section><section className="generator-shell"><div className="generator-choices"><ChoiceGroup label="Subject" options={subjects} selected={choices.subject} onChoose={(value) => choose('subject', value as Subject)} /><ChoiceGroup label="Difficulty" options={difficulties} selected={choices.difficulty} onChoose={(value) => choose('difficulty', value as Difficulty)} /><ChoiceGroup label="Medium" options={mediums} selected={choices.medium} onChoose={(value) => choose('medium', value as Medium)} /><ChoiceGroup label="Skill focus" options={skills} selected={choices.skills} multi onChoose={(value) => toggleSkill(value as Skill)} /><ChoiceGroup label="Available time" options={lengths} selected={choices.time} onChoose={(value) => choose('time', value as PracticeLength)} /><button className="generate-practice" aria-label="Generate my practice" onClick={generate}>Generate my practice <span>↗</span></button></div>{session ? <SessionBrief session={session} remaining={remaining} status={status} guidanceOpen={guidanceOpen} saved={saved} onStart={() => setStatus('running')} onPause={() => setStatus('paused')} onRestart={restart} onAddTime={() => setRemaining((value) => value + 600)} onFinish={() => setStatus('finished')} onToggleGuidance={() => setGuidanceOpen((open) => !open)} onSave={() => setSaved(true)} /> : <aside className="generator-empty"><span>✦</span><p>Your brief will arrive here.</p><small>One reference. One intention. A little protected time.</small></aside>}</section></AppShell>;
}
